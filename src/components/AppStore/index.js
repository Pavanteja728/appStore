...
class AppStore extends Component {
  ...
  filterClick = () => {
    ...
    const filterData = appsList.filter(each => each.category === activeTabId)
    return filterData
  }
  ...
  render() {
    ...
    const filterData = this.filterClick()
    const searchResults = filterData.filter(each =>
      each.appName.toLowerCase().includes(searchInput),
    )

    return (
      <div className="bgCont">
        <h1 className="head searchImg">App Store</h1>
